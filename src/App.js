...
function App() {
  const [tours, setTours] = useState(startingTours)

  const remove = id => {
    setTours(old => old.filter(tour => tour.id !== id))
  }

  const refresh = () => {
    setTours(startingTours)
  }

  const toursJSX = <><div id='top'>Our Tours</div>
      <div id='line'/>
      <TourList tours={tours} remove={remove} /></>

  const refreshJSX = <div id='no-tours-container'>
    <h1 id='no-tours-left'>No Tours Left</h1>
    <button id='refresher' onClick={refresh}>Refresh</button>
  </div>

  return (
    <div className="App">
      {tours.length === 0 ? refreshJSX : toursJSX}
    </div>
  );
}
